...
import edu.nwmissouri.isl.aasis.case.primary.persona.AbstractBaseControlComponent;
...
public class Participant extends AbstractBaseControlComponent implements IBaseControlComponentSlave {
   ...
   */
  public Participant(final String name, final IPersona persona, final Element knowledge,
      final OrganizationFocus focus) {
    super(name, persona, knowledge, focus);
    LOG.info("Entering constructor Participant( name={}, persona={}, knowledge={}, focus={}.", name, persona, knowledge,
        focus);
    ...
    try {
      this.playerCapability = new PlayableCapability(
          this.getPersonaExecutionComponent().getUniqueIdentifier().toString());
      this.organizationEvents = new OrganizationEvents(
          this.getPersonaExecutionComponent().getUniqueIdentifier().toString());
      if (debug)
        LOG.debug("Created org events for control component master: {}.", name);
  ...
  public void executeControlComponentPlan() {
    if (debug)
      LOG.info("EXECUTING Internal Subagent Participant plan. STATE={} under {}", this.state, this.masterOrgName);
    if (this.playerCapability.getPaused()) {
      if (debug)
        LOG.debug("PAUSED In CC SLAVE {} with master org {}", this.state, this.masterOrgName);
      return;
    }
  ...
  public synchronized void updateAgentInformation() {
    ...
    if (agent != null) {
      ...
      if (debug)
        LOG.debug("updateAgentInformation() AttributeAgent: {} with {}", agent.getIdentifier(),
            agent.getPossessesSet());
      final RegistrationContent registrationContent = new RegistrationContent(agent.getIdentifier(), capabilities);
      internalOrganizationCommunicationCapability
          .sendLocal(new ParticipateMessage(getPersonaExecutionComponent().getUniqueIdentifier(), this.master,
              ParticipatePerformative.AGENT_UPDATE_INFORMATION, registrationContent));
    }
  }
  ...
  protected synchronized void doInitialization() {
    if (debug)
      LOG.debug("Entering doInitialization()");
  ...
  protected synchronized void doRegistration() {
    if (debug)
      LOG.debug("Entering doRegistration()");
    ...
    if (debug)
      LOG.debug("In doRegistration. Agent= {}.  Possesses={}. ", id, caps);

    final RegistrationContent registrationContent = new RegistrationContent(
        getPersonaExecutionComponent().getUniqueIdentifier(), capabilities);
    if (debug)
      LOG.debug("registration content: {}", registrationContent);
    if (debug)
      LOG.debug("My org is {}.", this.masterOrgName);
    ...
    if (debug)
      LOG.debug("running on host {}.", host);
    if (debug)
      LOG.debug("in organization {} running on node {}", this.masterOrgName, host);
    ...
    final ParticipateMessage reasoningMessage = new ParticipateMessage(
        getPersonaExecutionComponent().getUniqueIdentifier(), StringIdentifier.getIdentifier("broadcast"),
        ParticipatePerformative.BROADCASTING_AGENT_REGISTRATION, registrationContent);
    if (debug)
      LOG.debug("reasoning message: {}", reasoningMessage);
    ...
    if (reasoningMessage != null) {
      try {
        if (this.internalOrganizationCommunicationCapability.broadcastIncludeSelf(reasoningMessage)) {
          ...
          LOG.info(
              "EVENT: SUCCESSFUL_REGISTRATION_MESSAGE broadcast. {} messages on queue.  Moving to {}.  MESSAGE={}. internal org messages are: {}",
              this.internalOrganizationCommunicationCapability.getInternalOrganizationMessages().size(), this.state,
              reasoningMessage, this.internalOrganizationCommunicationCapability.getInternalOrganizationMessages());
  ...
  protected synchronized void doRegistrationConfirmation() {
    if (debug)
      LOG.debug("Entering doRegistrationConfirmation()");
    if (this.internalOrganizationCommunicationCapability.messages() > 0) {
      final ParticipateMessage message = (ParticipateMessage) this.internalOrganizationCommunicationCapability
          .receiveLocal();
      ...
      case AGENT_REGISTRATION_CONFIRMATION:
        LOG.info("EVENT: ORGANIZATION_PARTICIPANT_REGISTERED.  received registration confirmation. content = {}",
            message.getContent());
        ...
        if (content instanceof UniqueIdentifier) {
          ...
          LOG.debug("doRegistrationConfirmation()confirmation from {}. Sending master conf received message.",
              this.master);
          this.internalOrganizationCommunicationCapability
              .sendLocal(new ParticipateMessage(getPersonaExecutionComponent().getUniqueIdentifier(), this.master,
      ...
      default:
        ...
        LOG.error("Unacceptable performative in doRegistrationConfirmation: {}. content = {}",
            message.getPerformativeType(), message.getContent().toString());
        System.exit(-19);
      ...
      if (debug)
        LOG.debug("In CC SLAVE confirming registration, but no messages to process.");
      this.state = ExecutionState.TIMING_OUT_DURING_CONFIRMATION;
    }
  }
  ...
  protected synchronized void doRegistrationConfirmationTimeout() {
    if (debug)
      LOG.debug("Entering doRegistrationConfirmationTimeout()");
    ...
    if (waited > confirmation_timeout_ms) {
      ...
      if (debug)
        LOG.debug("doRegistrationConfirmationTimeout() {}", String.format("Waiting Done: ((%d - %d) = %d) > %d",
            currentTime, this.startTime, currentTime - this.startTime, confirmation_timeout_ms));
      if (debug)
        LOG.debug("Going back to registering.");
      this.state = ExecutionState.REGISTERING;
    } else {
      if (debug)
        LOG.debug("Going to confirming registration.");
      this.state = ExecutionState.CONFIRMING_REGISTRATION;
    }
  }
  ...
  protected synchronized void doRegistrationRetry() {
    if (debug)
      LOG.debug("Entering doRegistrationRetry()");
    ...
    if (timeDifference > retry_delay_ms) {
      if (debug)
        LOG.debug("Waited for getParticipantRetryDelay_ms. Will try again. doRegistrationRetry() {}", String.format(
            "Waiting Done: ((%d - %d) = %d) > %d", currentTime, this.startTime, timeDifference, retry_delay_ms));
      this.state = ExecutionState.REGISTERING;
    } else { // continue to wait
      if (debug)
        LOG.debug("SELF CCS doRegistrationRetry() {}", String.format("Continue Waiting: ((%d - %d) = %d) <= %d",
  ...
  protected synchronized void doWork() {
    ...
    if (this.playerCapability.getPaused()) {
      if (debug)
        LOG.debug("PAUSED In CC SLAVE STATE doWork()");
      return;
    }
    ...
    while (this.organizationEvents.numberOfQueuedEvents() > 0) {
      ...
      if (debug)
        LOG.debug("numberOfQueuedEvents {}",
            String.format("processing OrganizationEvent:" + " %s", organizationEvents));
      processEvent(organizationEvents);
    }
    if (debug)
      LOG.debug("Checking internal messages.");
  ...
  protected synchronized void processEvent(final List<IOrganizationEvent> organizationEvents) {
    if (debug)
      LOG.debug("Entering processEvent(organizationEvents={})", organizationEvents);
    if (this.master == null) {
      ...
      while (!this.queuedOrganizationEvents.isEmpty()) {
        ...
        if (debug)
          LOG.debug("Sending Previously Queued OrganizationEvent: {}", priorEvents);
        final ListEventsContent content = new ListEventsContent(OrganizationEventContent.convert(priorEvents));
        final ParticipateMessage reasoningMessage = new ParticipateMessage(
            getPersonaExecutionComponent().getUniqueIdentifier(), this.master, ParticipatePerformative.GOAL_MODEL_EVENT,
            content);
        internalOrganizationCommunicationCapability.sendLocal(reasoningMessage);
      }
      if (debug)
        LOG.debug("processEvent() Sending Current OrganizationEvent:{}", organizationEvents);
      final ListEventsContent content = new ListEventsContent(OrganizationEventContent.convert(organizationEvents));
      this.internalOrganizationCommunicationCapability
          .sendLocal(new ParticipateMessage(getPersonaExecutionComponent().getUniqueIdentifier(), this.master,
              ParticipatePerformative.GOAL_MODEL_EVENT, content)); // added to force local send
    }
  }

  protected synchronized void processMessage(final ParticipateMessage message) {
    if (debug)
      LOG.debug("Entering processMessage(message={})", message);
    if (message != null) {
      switch (message.getPerformativeType()) {
      case BROADCASTING_AGENT_REGISTRATION:
        if (debug)
          LOG.debug("processMessage() {}", String.format("Discarding Agent Registration: %s", message.toString()));
        break;
      case AGENT_REGISTRATION_CONFIRMATION:
        if (debug)
          LOG.debug("processMessage() {}", String.format("Discarding Agent Registration: %s", message.toString()));
        break;
      case ASSIGNMENT:
        LOG.info("EVENT: ASSIGNMENT_RECEIVED. Name = {} received assignment. content = {}",
            this.getOwner().getUniqueIdentifier().toString(), message.getContent());
      ...
      default:
        ...
        LOG.error("Unacceptable performative in processMessage: {}. content = {}", message.getPerformativeType(),
  ...
  protected synchronized void triggerReorganization() {
    if (debug)
      LOG.debug("Entering triggerReorganization(})");
    if (this.reorganize) {
  ...
  protected void reorganize() {
    ...
    if (assignments.isEmpty()) { // no new ec
      if (debug)
        LOG.debug("reorganize() {}", "No New Assignments");
    } else { // there are new ec
      if (debug)
        LOG.debug("{} New Assignments: {}", assignments.size(), assignments);
      ...
      for (final Assignment assignment : assignments) {
        ...
        internalOrganizationCommunicationCapability.sendLocal(new ParticipateMessage(
            getPersonaExecutionComponent().getUniqueIdentifier(), assignment.getAgent().getIdentifier(),
  ...
  protected enum ExecutionState {
    CONFIRMING_REGISTRATION, INITIALIZING, REGISTERING, RETRYING_REGISTRATION, TIMING_OUT_DURING_CONFIRMATION, WORKING
  }
}
