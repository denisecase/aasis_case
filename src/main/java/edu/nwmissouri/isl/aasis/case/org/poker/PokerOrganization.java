 ...
 *
 */
package edu.nwmissouri.isl.aasis.case.org.poker;
...
import edu.nwmissouri.isl.aasis.case.org.poker.guidelines.PokerGuidelineManager;
 ...
 */
public class PokerOrganization extends Organization {
  private static final Logger LOG = LoggerFactory.getLogger(PokerOrganization.class);
   ...
   */
  public PokerOrganization(final String folder, final OrganizationFocus focus, String orgModelFolder) {
    super(folder, focus, orgModelFolder);
    init();
  }

  public PokerOrganization(String folder, OrganizationFocus focus, String orgModelFolder, String goalFilePath,
      String roleFilePath, String topGoal) {
  ...
  public synchronized void init() {
     if (debug)
      LOG.debug("Exiting constructor: {}", this.getName());
  }
  ...
  public Collection<IPersona> getChildren() {
    if (debug)
      LOG.debug("{} total agents in {} organization.", this.getAllPersona().size(), this.getName());
    ...
    Collection<IPersona> c = this.getAllPersona().stream()
        .filter(persona -> persona.getClass().getSimpleName().equals("HomePersona")).collect(Collectors.toList());
    return c;
  }
  ...
  public void loadTopGoalGuidelines() {
    LOG.info("Begin reading top goal guidelines for {}. "
        + "The guidelines should match the top level goal parameters shown on the goal model.", this.getName());

    // get the source file (initialize.xml)

    String absPathToFile = this.getOrganizationSpecification().getGoalParametersFile();
    if (debug)
      LOG.debug("Initial top goal guidelines will be read from {}", absPathToFile);

    this.goalParameterValues = PokerGuidelineManager.getGuidelines(absPathToFile);
    LOG.info("Organization guidelines (i.e. top goal parameter values) set for {}. {}", this.getName(),
        this.goalParameterValues);
  }
  ...
  public void loadInitialGoalGuidelines(Map<UniqueIdentifier, Object> goalParameterValues) {
    if (debug)
      LOG.info("Begin setting guidelines for external org {}.", this.getName());
    if (debug)
      LOG.debug("Initial goal parameters set from {}", goalParameterValues);
    if (debug)
      LOG.debug("{} items in the goal parameter values key set.", goalParameterValues.size());
    ...
    for (UniqueIdentifier key : goalParameterValues.keySet()) {
      ...
      if (debug)
        LOG.debug("Getting input goal guideline. Key= {} Value={}", key, goalParameterValues.get(key));
    }

    IConnections pokerConnections = (IConnections) this.getGoalParameterValues()
        .get(PokerGoalParameters.pokerConnections);
    ...
    for (IConnectionGuidelines cg : pokerConnections.getListConnectionGuidelines()) {
    ...
    if (orgModelFolder.isEmpty()) {
      LOG.info(
          "The folder under the standard models directory is not available. No default goal and role models for this organization can be used. A goal model and role model must be available with the Agent.xml file. ");
    } else {
      this.getOrganizationSpecification().setOrgModelFolder(orgModelFolder);
    }

    IPokerPlayerGuidelines ag = PokerPlayerGuidelinesBuilder.createGuidelines(); // defaults
    this.getGoalParameterValues().put(PokerGoalParameters.pokerPlayerGuidelines, ag);
    if (debug)
      LOG.info("Set auction guidelines for {} to {}.", this.getName(), ag);

    IPokerGuidelines bg = PokerGuidelinesBuilder.create(); // defaults all
    this.getGoalParameterValues().put(PokerGoalParameters.pokerGuidelines, bg);
    if (debug)
      LOG.info("Set broker guidelines for {} to {}.", this.getName(), bg);
  }
  ...
  public void initializeGoals() {
    ...
    allPersona.stream()
        .filter(p -> p.getPersonaControlComponent().getClass().toString().contains("ControlComponentMaster"))
        .forEach(p -> {
  ...
  public void run() {
    if (debug)
      LOG.debug("Starting organization turn {} of external org {}.", turns, this.getName());
    if (this.terminated.get()) {
      if (debug)
        LOG.debug("Terminated - cleaning up.");
    ...
    try {
      if (debug)
        LOG.debug("Getting the agents specified (see Agent.xml) in external organization {} " + "under local master {}",
            this.getName(), this.getLocalMaster());
      /*
      ...
      */
      ...
      if (!this.personaWaitingToJoin.isEmpty()) {
        ...
        while (!this.personaWaitingToJoin.isEmpty()) {
          if (debug)
            LOG.debug("Agents waiting to join external organization {}.", this.getLocalMaster());
          final IPersona persona = this.personaWaitingToJoin.poll();
          if (debug)
            LOG.info("Waiting to join organization {} persona: {}.", this.getName(), persona.getIdentifierString());
          /*
        ...
        for (final IPersona p : newParticipants) {
          ...
          final Thread thread = new Thread(p,
              String.format("%1$30s", this.getName() + "." + String.format("%1$15s", p.getUniqueIdentifier() + "ORG")));
          if (debug)
            LOG.info("ORG: CC local master = {} this persona = {}", p.getPersonaControlComponent().getLocalMaster(),
                p.getUniqueIdentifier());
          if (debug)
            LOG.info("Starting new participant thread in external organization {} for CC-EC pair {}-{}.",
                this.getName(), p.getPersonaControlComponent().getLocalMaster(), p.getUniqueIdentifier());
          thread.start();
  ...
  public void initialize(final File folder) {
    if (debug)
      LOG.info("Initializing external org {}", folder.getName());
