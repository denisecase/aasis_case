...
/**
 * Poker participation communication capability.
 */
public class PokerParticipateCapability extends AbstractOrganizationCapability
    implements IOrganizationCommunicationCapability, ICapability {
  private static final Logger LOG = LoggerFactory.getLogger(PokerParticipateCapability.class);
  ...
  */
  ...
  public PokerParticipateCapability(final IPersona owner, IOrganization organization) {
    super(PokerParticipateCapability.class, owner, organization);
  ...
  public String toString() {
    return "IOrganizationCommunicationCapability{" + "organizationMessages=" + organizationMessages + ", owner=" + owner
        + '}';
  }
  ...
  public boolean send(final IBaseMessage<?> message) {
    ...
    if (isLocal) {
      if (debug)
        LOG.debug("Attempting to send on channel {} to receiver {} local message {}", getCommunicationChannelID(),
            message.getLocalReceiver(), message);
      ...
      String routingKey = PokerMessagingManager.getFullQueueName(message.getRemoteReceiver(),
    ...
    if (!success) {
      LOG.error("ERROR attempting to send on channel {} to receiver {} local message {}", getCommunicationChannelID(),
          message.getLocalReceiver(), message);
      System.exit(-2221);
    }
    return success;
  ...
  protected boolean broadcast(final BaseMessage<?> message) {
    Objects.requireNonNull(getOwner(), "execution component cannot be null");
    Objects.requireNonNull(getOwner().getCapability(IInternalCommunicationCapability.class),
        "capability cannot be null");
    Objects.requireNonNull(getCommunicationChannelID(), "communication channelID cannot be null");
    try {
      return getOwner().getCapability(IInternalCommunicationCapability.class).broadcast(getCommunicationChannelID(),
          message);
    } catch (Exception e) {
   ...
   * Object)
   */
  protected boolean broadcastIncludeSelf(final BaseMessage<?> message) {
  ...
  public synchronized boolean doRegistration() {
    ...
    if (debug)
      LOG.debug("{} doRegistration() {}", getClass().getName(),
          String.format("Agent: \"%s\": %s", owner.getIdentifierString(), owner.getCapabilitiesMapping().toString()));

    final RegistrationContent registrationContent = new RegistrationContent(this.owner.getUniqueIdentifier(),
        capabilities);
    if (debug)
      LOG.debug("registration content: {}", registrationContent.toString());
    ...
    if (debug)
      LOG.debug("This CC Slave  is  {} in organization {} running on node {}", this.owner.getOrganization().getName(),
          org, host);
    ...
    if (debug)
      LOG.debug("Creating participate message. The remote org to join={}, this master={}, orgmaster={}", org,
          this.masterName, StringIdentifier.getIdentifier(org));
    final ParticipateMessage reasoningMessage = new ParticipateMessage(this.owner.getUniqueIdentifier(),
        ...
        registrationContent);
    if (debug)
      LOG.debug("Created registration message: {}", reasoningMessage.toString());
  ...
  public List<? extends IConnectionGuidelines> getUnregisteredParents() {
    ...
    if (debug)
      LOG.debug("Need {} of {} registrations to fully connect to participants.", stillNeeded, tot);
    return unregistered;
  }
  ...
  public synchronized void init(final InstanceGoal<?> instanceGoal) {
    ...
    if (IPokerPlayerGuidelines.extractGuidelines(params) != null) {
      //            this.setPokerPlayerGuidelines(IPokerPlayerGuidelines.extractPokerPlayerGuidelines(params));
      //            LOG.info("e{}.", this.PokerPlayerGuidelines);
      //            if (this.PokerPlayerGuidelines == null) {
      //                LOG.info("Auction guidelines are null. params={}", params);
      //            }
      this.parentConnections = (IConnections) params.getParameters().get(PokerGoalParameters.pokerDealerConnections);
  ...
  private void extractParentConnections(InstanceParameters params) {
    ...
    if (debug)
      LOG.debug("Initializing broker connections: {}.", parentConnections);

    if (parentConnections == null) {
      if (debug)
        LOG.debug("There are no broker connections to other agents.");
    }
  ...
  public IBaseMessage<?> receiveLocal() {
    if (debug)
      LOG.debug("Checking for internal organization messages");
    return organizationMessages.poll();
  }

  public boolean sendLocal(final BaseMessage<?> message) {
    if (debug)
      LOG.debug("Attempting to send local message {}", message);
    if (debug)
      LOG.debug("Attempting to broadcast local message on com channel (filter)={} with content={}. Message={}",
          getCommunicationChannelID(), message.getContent(), message);
  ...
  public synchronized boolean sendExternal(final IBaseMessage<?> message) {
    ...
    LOG.debug("  entering send local messages externally from {} to {}", message.getLocalSender(),
        message.getLocalReceiver());
    String routingKey = message.getLocalReceiver().toString(); // e.g. N43inN43
    MessagingManager.declareAndBindConsumerQueue(messagingFocus,
        PokerMessagingManager.getFullQueueName(routingKey, PokerMessagingManager.getQueueFocus(messagingFocus)));
    try {
