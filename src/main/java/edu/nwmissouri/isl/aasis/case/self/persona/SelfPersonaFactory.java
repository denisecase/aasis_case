package edu.nwmissouri.isl.aasis.case.self.persona;
...
import edu.nwmissouri.isl.aasis.case.primary.persona.factory.PersonaFactory;
...
public class SelfPersonaFactory extends PersonaFactory {
  ...
  public static void setupPersonaList(NodeList agentsList, IOrganization organization) {
    ...
    for (int i = 0; i < agentsList.getLength(); i++) {
      if (debug)
        LOG.debug("");
      if (debug)
        LOG.debug("{}: getting constructors for persona type {} of {}.", organization.getName(), i + 1,
            agentsList.getLength());
      ...
      final AttributeConstructorValues[] attributes = setupAttributes(
          agentsElement.getElementsByTagName(ELEMENT_ATTRIBUTE));

      final ICapabilityConstructorValues[] capabilityValues = setupCapabilities(
          agentsElement.getElementsByTagName(ELEMENT_CAPABILITY), organization);
      ...
      for (int j = 0; j < personaList.getLength(); j++) {
        ...
        if (debug)
          LOG.debug("Instantiating persona {}. Getting {} constructor.", agentName, agentType);

        Constructor<? extends IPersona> sec_con = SelfPersonaFactory.getExecutionComponentConstructor(
            SelfPersonaFactory.getExecutionComponentClass(stringPackage + "." + agentType));
        if (debug)
          LOG.debug("This self organization's top goal is: {}",
        ...
        if (debug)
          LOG.debug("This agent's internal guidelines are: {}. Beginning creation of execution component. ",
              organization.getGoalParameterValues());

        // create a new Execution Component IAgent for this persona (which has a CC either Master or Slave).............................
        if (debug)
          LOG.debug("Calling createEC org={}, orgEl={}, attribs={}, capValues={}, agentname={}, sec_con={}.",
              organization, orgElement, attributes, capabilityValues, agentName, sec_con);
        createEC(organization, orgElement, attributes, capabilityValues, agentName, sec_con);
        LOG.info("EVENT: SUB-AGENT_LOADED. agentOrg={}, aubAgentName={} added with EC-CC pair.", organization.getName(),
            agentName);
  ...
  protected synchronized static void createEC(IInnerOrganization organization, Element orgElement,
      AttributeConstructorValues[] attributes, ICapabilityConstructorValues[] capabilityValues, String agentName,
      Constructor<? extends IPersona> sec_con) {
    LOG.info(
        "Entering PersonaFactory.createEC(personaorg={}, orgElement={}, attributes={}, capconstructorvalues={}, agentName={}, sec_con).)",
        organization, orgElement, attributes, capabilityValues, agentName, sec_con);
    final IPersona persona = Objects.requireNonNull(
        newExecutionComponent(sec_con, organization, agentName, orgElement, OrganizationFocus.Agent),
    ...
    if (debug)
      LOG.debug("Getting {} EC capability constructor values.", capabilityValues.length);
    for (final ICapabilityConstructorValues v : capabilityValues) {
      if (debug)
        LOG.debug("\tAdding: {}", v.getConstructor());
      v.getParameters()[0] = persona;
      if (debug)
        LOG.debug("Adding capability {} to execution_component {}.", v.getConstructor(), persona.getIdentifierString());
  ...
  public static Class<? extends ICapability> getCapabilityActionClass(final String className) {
    if (debug)
      LOG.debug("In getCapabilityActionClass looking up ICapability derived class for {}.", className);
    try {
      if (debug)
  ...
  public static Constructor<? extends ICapability> getCapabilityActionConstructor(
      final Class<? extends ICapability> capabilityClass, final Class<?>... parameterTypes) {
  ...
  public static Class<? extends IPersona> getExecutionComponentClass(final String className) {
    if (debug)
      LOG.debug("getting execution component class for {}", className);
    try {
      return Class.forName(className).asSubclass(IPersona.class);
    } catch (final ClassNotFoundException e) {
      LOG.error("ERROR: ClassNotFoundException for className={}.", className);
      System.exit(-91);
    }
    return null;
  }
  ...
  public static Constructor<? extends IPersona> getExecutionComponentConstructor(
    ...
    if (debug)
      LOG.debug("Getting execution component constructor for {}.", executionComponentClass);
    Constructor<? extends IPersona> constructor = null;
    try {
      constructor = executionComponentClass.getConstructor(IOrganization.class, String.class, Element.class,
          OrganizationFocus.class);
    } catch (Exception e) {
      LOG.error("ERROR: getting execution component constructor for {}.", executionComponentClass, e);
      System.exit(-91);
    }
    return constructor;
  }
   ...
   */
  public static ICapability newCapability(final Constructor<? extends ICapability> constructor,
   ...
   */
  public static IPersona newExecutionComponent(Constructor<? extends IPersona> constructor, IOrganization organization,
      String identifier, Element knowledge, OrganizationFocus focus) {
