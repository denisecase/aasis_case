...
import edu.nwmissouri.isl.aasis.case.org.poker.PokerOrganizationFactory;
...
/**
 * The {@code PokerAdminCapability} provides the functionality necessary to create a new organization.
 */
public class PokerAdminCapability extends AbstractOrganizationCapability {
    private static final Logger LOG = LoggerFactory.getLogger(PokerAdminCapability.class);
    private static final boolean debug = false;
    private static final IMessagingFocus messagingFocus = PokerMessagingFocus.POKER_PARTICIPATE;
    ...
    /**
     * Construct a new {@code POKERAdminCapability} instance.
     *
     ...
     */
    public PokerAdminCapability(final IPersona owner, final IOrganization org) {
        super(PokerAdminCapability.class, owner, org);
    ...
    public IPokerGuidelines getPokerGuidelines() {
    ...
    public synchronized void init(InstanceGoal<?> ig) {
        ...
        if (this.tictactoeConnections == null) {
            LOG.error("Broker trying to initialize an external organization with no  participants. params={}", params);
            System.exit(-56);
        } else {
            if (debug) LOG.info("Starting initialization of new organization by {}. ", this.getOwner());
            if (debug)
                LOG.info("{} authorized connections to participants.", tictactoeConnections.getListConnectionGuidelines().size());
    ...
    public synchronized IOrganization createOrganization(InstanceGoal<?> ig) {
        ...
        try {
            this.myorg = new PokerOrganizationFactory().createBaseAffiliateOrganization(folder.getAbsolutePath(),
                    OrganizationFocus.External, orgParams,
                    this.orgModelFolder, goalFilePath, roleFilePath, RunManager.getTopGoal());
            LOG.info("Poker org initialized with agents and top goal guidelines. Standard models in {} subfolder.", myorg.getOrgModelFolder());

        }
        catch (Exception e) {
            LOG.error("ERROR: Could not create organization.");
            return null;
        }
        return this.myorg;
    }
    ...
    private HashMap<UniqueIdentifier, Object> extractNewOrganizationGuidelines(InstanceGoal<?> ig) {
        ...
        for (final Map.Entry<UniqueIdentifier, Object> entry : params.getParameters().entrySet()) {
            ...
            if (id.toString().contains("PokerGuidelines") || id.toString().contains("connections")) {
                newMap.put(id, value);
            }
        }
        return newMap;
    ...
    private boolean areThereEnoughAgentsForInitialAssignments() {
        if (debug) LOG.debug("In POKERAdminCapability of {} " +
    ...
    private boolean remoteSendAssignmentMessage(edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipateMessage message) {
        ...
        final String fullQueueName = PokerMessagingManager.getFullQueueName(receiver, PokerMessagingManager.getQueueFocus(messagingFocus));
    ...
    public Set<Agent<?>> getLocalRegisteredProsumers(Set<Agent<?>> allAgents) {
        ...
        if (debug) LOG.debug("Number of all agents found in the POKERAdminCapability is {}",
                allAgents.size());

        final Set<Agent<?>> prosumers = new HashSet<>();
        Iterator<Agent<?>> it = allAgents.iterator();

        final Class<?> capabilityClass = PokerCapability.class;
    ...
    public synchronized void initializeChildConnections(InstanceGoal<?> instanceGoal) {
        ...
        if (debug)
            LOG.debug("There are {} authorized connections to participants.", tictactoeConnections.getListConnectionGuidelines().size());
    }
    ...
    public synchronized ParticipateMessage remoteRECEIVE(final String queueLink) throws IOException, ShutdownSignalException, InterruptedException {
        final String fullQueueName = PokerMessagingManager.getFullQueueName(queueLink, PokerMessagingManager.getQueueFocus(messagingFocus));
    ...
    private void setConsumer(String queueLink) throws IOException {
        ...
        String fullQueueName = PokerMessagingManager.getFullQueueName(queueLink, PokerMessagingManager.getQueueFocus(messagingFocus));
