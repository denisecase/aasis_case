...
public class SelfControlCapability extends AbstractOrganizationCapability {
  ...
  public UniqueIdentifier findParticipant(String participantCapability, String orgIdentifier) {
    ...
    try {
      ...
      LOG.error("ERROR: trying to find participant but could not get class for participant capability = {}.",
  ...
  public Set<Agent<?>> getLocalRegisteredProsumers(Set<Agent<?>> allAgents) {
    ...
    if (debug)
      LOG.debug("Number of all agents found in the SelfControlCapability is {}", allAgents.size());
    ...
    while (it.hasNext()) {
      Agent<?> agent = it.next();
      if (debug)
        LOG.debug("Checking registered agent {} for SelfControlCapability", agent.toString());
  ...
  public String toString() {
    return "SelfControlCapability{" + "initialGuidelines=" + initialGuidelines + '}';
  }
