...
public enum Self_Control implements IPlanState<Self_Control_Plan> {
  ...
  public synchronized void Execute(final IExecutablePlan plan, final IExecutor ec, final InstanceGoal<?> ig) {
    ...
    Objects.requireNonNull(ec.getCapability(ITicTacToeConnectCapability.class),
        "Role requires ITicTacToeConnectCapability.");
    ...
    //     // IPowerMessage localPowerMessage = ec.getCapability(IPokerCommunicationCapability.class).checkForLocalPowerMessageFromWorker();
    ...
    //     //         boolean success = ec.getCapability(IPokerCommunicationCapability.class).forwardToParticipant(localPowerMessage, subIdentifier);
    ...
    //     //     IPowerMessage m = ec.getCapability(IPokerCommunicationCapability.class).checkFromAdmin(ec.getUniqueIdentifier().toString());
    ...
    //     //         UniqueIdentifier subIdentifier = ec.getCapability(SelfControlCapability.class).findParticipant(TicTacToeParticipateCapability.class.getCanonicalName(),"");
    //     //         if (debug) LOG.debug("The sub holon persona is {}.", subIdentifier);
    //     //         if (subIdentifier != null) {
    ...
    //     //             boolean success = ec.getCapability(IPokerCommunicationCapability.class).forwardToParticipant(lmsg, subIdentifier);
    ...
    // if (null == ec.getCapability(IPokerCommunicationCapability.class)) return;

    // if (!myPersona.startsWith("selfH")) {
    //     LOG.debug("Not the lowest level. Will initialize capabilities from instance goal = {}.", ig);
    //     ec.getCapability(IPokerCommunicationCapability.class).init(ig);
    //     LOG.debug("Set IPokerCommunicationCapability guidelines from instance goal. {}", ig);

    //     // if (debug) LOG.debug("Self checking for market messages to forward.");
    //     // IAuctionMessage m = ec.getCapability(IPokerCommunicationCapability.class).checkFromAdmin(myPersona);
    //     // if (m == null) return;
    ...
    //     //     if (!ec.getCapability(IPokerCommunicationCapability.class).isTopTier()) {
    ...
    //     //         UniqueIdentifier subIdentifier = ec.getCapability(SelfControlCapability.class).findParticipant(PokerParticipateCapability.class.getCanonicalName(), "A");
    ...
    //     //         boolean success = ec.getCapability(IPokerCommunicationCapability.class).sendDownResults(m);
