...
import edu.nwmissouri.isl.aasis.case.org.tictactoe.TicTacToeOrganizationFactory;
import edu.nwmissouri.isl.aasis.case.org.tictactoe.capabilities.participate.TicTacToeParticipateCapability;
import edu.nwmissouri.isl.aasis.case.org.tictactoe.messages.TicTacToePerformative;
...
/**
 The {@code TicTacToeAdminCapability} provides the functionality necessary to create a new organization.
 */
public class TicTacToeAdminCapability extends AbstractOrganizationCapability {
    private static final Logger LOG = LoggerFactory.getLogger(TicTacToeAdminCapability.class);
    private static final boolean debug = false;
    private static final IMessagingFocus messagingFocus = TicTacToeMessagingFocus.TICTACTOE_PARTICIPATE;
    private static Channel channel = TicTacToePerformative.MessagingManager.getChannel(messagingFocus);

    private Map<UniqueIdentifier, Agent<UniqueIdentifier>> registeredAgentQueue = new ConcurrentHashMap<>();
    ...
    /**
     Construct a new {@code TicTacToeAdminCapability} instance.
     ...
     */
    public TicTacToeAdminCapability(final IPersona owner, final IOrganization org) {
        super(TicTacToeAdminCapability.class, owner, org);
    ...
    public synchronized void setChildConnections(final IConnections tictactoeConnections) {
    ...
    public synchronized void setRegisteredAgentQueue(Map<UniqueIdentifier, Agent<UniqueIdentifier>> registeredAgentQueue) {
    ...
    public String toString() {
        return "TicTacToeAdminCapability{" +
                "doneWaitingForInitialAssignments=" + doneWaitingForInitialAssignments +
                ", initialRoles=" + initialRoles +
                ", tictactoeConnections=" + tictactoeConnections +
                ", myorg=" + myorg +
                '}';
    }
    ...
    public IOrganization createAndStartNewOrganization(InstanceGoal<?> ig) {
        this.init(ig);
    ...
    public void init(InstanceGoal<?> ig) {
        ...
        this.initializeChildConnections(ig);
        if (this.tictactoeConnections == null) {
            LOG.error("Parent trying to initialize an external organization with no participants. Guidelines={}", params);
            System.exit(-59);
        } else {
            if (debug) LOG.info("Starting initialization of new organization by {}. ", this.getOwner());
            this.setChildConnections(tictactoeConnections);
            if (debug)
                LOG.debug("{} authorized connections to sub holons.", tictactoeConnections.getListConnectionGuidelines().size());
            if (debug) LOG.info("Starting initialization of holonic grid control org by {}. ", this.getOwner());
    ...
    public IOrganization createOrganization(InstanceGoal<?> ig) {
        // analogous to launcher......
        File folder = new File(RunManager.getAbsolutePathToTestCaseFolder() + "/" +
                tictactoeConnections.getListConnectionGuidelines().get(0).getSpecificationFilePath());
        if (debug) LOG.debug("The organization information is in {}", folder);
        this.orgModelFolder = tictactoeConnections.getListConnectionGuidelines().get(0).getOrgModelFolder();
        String orgName = folder.getName();
        if (debug) LOG.debug("The organization name is {}", folder);
        if (this.orgModelFolder.isEmpty()) {
            LOG.error("ERROR: could not get path the folder with this orgs standard models. ");
            System.exit(-23);
        }
        if (debug)
            LOG.debug("Standard organization goal and role models are in the standard models {} folder.", this.orgModelFolder);

        String goalFilePath = RunManager.getAbsolutePathToStandardOrganizationGoalModel(this.orgModelFolder);
        String roleFilePath = RunManager.getAbsolutePathToStandardOrganizationRoleModel(this.orgModelFolder);
        HashMap<UniqueIdentifier, Object> orgParams = extractNewOrganizationGuidelines(ig);
        this.myorg = new TicTacToeOrganizationFactory().createBaseAffiliateOrganization(folder.getAbsolutePath(),
                OrganizationFocus.External, orgParams,
                this.orgModelFolder, goalFilePath, roleFilePath, RunManager.getTopGoal());
        LOG.info("TicTacToe org initialized with agents and top goal guidelines. Standard models in {} subfolder.", myorg.getOrgModelFolder());
        return this.myorg;
    }
    ...
    private HashMap<UniqueIdentifier, Object> extractNewOrganizationGuidelines(InstanceGoal<?> ig) {
        ...
        for (final Map.Entry<UniqueIdentifier, Object> entry : params.getParameters().entrySet()) {
            ...
            if (id.toString().contains("Connections") || id.toString().contains("Guidelines")){
                // TODO: takes 'em all - could be more limited
                newMap.put(id, value);
            }
        }
        return newMap;
    ...
    public synchronized IOrganization createExternalOrganization(InstanceGoal<?> ig) {
        if (debug) LOG.debug("Creating external organization from goal: {}.", ig);
        // Get the parameter values from the existing active instance goal
        final InstanceParameters params = Objects
                .requireNonNull((InstanceParameters) ig
                        .getParameter());
        ...
        if (this.tictactoeConnections == null) {
            ...
            if (debug)
                LOG.debug("{} authorized connections to sub holons.", tictactoeConnections.getListConnectionGuidelines().size());
            if (debug) LOG.info("Starting initialization of holonic org by {}. ", this.getOwner());

            final IConnectionGuidelines cg = tictactoeConnections.getListConnectionGuidelines().get(0);
            if (debug) LOG.info("Organization guidelines found at {}:  ", cg.getSpecificationFilePath());

            // analogous to launcher......
            File folder = new File(RunManager.getAbsolutePathToTestCaseFolder() + "/" + cg.getSpecificationFilePath());
            if (debug) LOG.debug("The organization information is in {}", folder);

            this.orgModelFolder = Objects
                    .requireNonNull((IConnections) params
                            .getValue(StringIdentifier
                                    .getIdentifier("tictactoeConnections"))).getListConnectionGuidelines().get(0).getOrgModelFolder();
            if (this.orgModelFolder.isEmpty()) {
                LOG.error("ERROR: could not get path the folder with this orgs standard models. ");
                System.exit(-22);
            }
            ...
            this.myorg = new TicTacToeOrganizationFactory().createBaseAffiliateOrganization(folder.getAbsolutePath(),
                    OrganizationFocus.External, orgParams, this.orgModelFolder);

            LOG.info("Holonic org initialized with agents and top goal guidelines. Standard models in {} subfolder.", myorg.getOrgModelFolder());
        }
        return this.myorg;
    }
    ...
    public boolean doWaitingForAgentsForInitialAssignments() {
        ...
        if (this.doneWaitingForInitialAssignments) {
            ...
            while (organizationCommunicationCapability.messages() > 0) {
                final ParticipateMessage message = (ParticipateMessage)
    ...
    private boolean areThereEnoughAgentsForInitialAssignments() {
        if (debug) LOG.debug("In TicTacToeAdminCapability of {} " +
        ...
        final HashSet<Role> rolesToRemove = new HashSet<>();
        ...
        for (final Role role : this.initialRoles) {
            this.organizationModel.getInstanceGoals().stream().filter(role::achieves).forEach(goal ->
                    rolesToRemove.addAll(this.organizationModel.getAgents().stream().filter(agent ->
                            role.goodness(agent, goal, new HashSet<>()) >
                                    RoleGoodnessFunction.MIN_SCORE).map(agent -> role).collect(Collectors.toList())));
        }
    ...
    protected void reorganize() {
        ...
        if (assignments == null) {
            if (debug) LOG.debug("reorganize() {}", "Reorganization Algorithm IUnreliable - assignment collection is null");
            return;
        }
    ...
    private boolean remoteSendAssignmentMessage(edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipateMessage message) {
        ...
        final String fullQueueName = TicTacToeMessagingManager.getFullQueueName(receiver, TicTacToeMessagingManager.getQueueFocus(messagingFocus));
        final String routingKey = fullQueueName;
        TicTacToeMessagingManager.declareAndBindConsumerQueue(messagingFocus, receiver);
        try {
            byte[] messageBodyBytes = "hello".getBytes();
            channel.basicPublish(TicTacToeMessagingManager.getExchangeName(messagingFocus), routingKey, MessageProperties.PERSISTENT_TEXT_PLAIN, messageBodyBytes);
    ...
    public Set<Agent<?>> getLocalRegisteredProsumers(Set<Agent<?>> allAgents) {
        ...
        if (debug) LOG.debug("Number of all agents found in the TicTacToeAdminCapability is {}",
                allAgents.size());

        final HashSet<Agent<?>> prosumers = new HashSet<>();
        Iterator<Agent<?>> it = allAgents.iterator();

        final Class<?> capabilityClass = TicTacToeParticipateCapability
                .class;
        ...
        while (it.hasNext()) {
            Agent<?> agent = it.next();
            if (debug) LOG.debug("Checking registered persona {} for AssessReactivePowerQualityCapability", agent.toString());
    ...
    public synchronized void initializeGuidelines(InstanceGoal<?> instanceGoal) {
        ...
        final IConnections tictactoeConnections = Objects
                .requireNonNull((IConnections) params
                        .getValue(StringIdentifier
                                .getIdentifier("tictactoeConnections")));
        this.setChildConnections(tictactoeConnections);
        if (debug)
            LOG.debug("There are {} authorized connections to sub holon agents.", tictactoeConnections.getListConnectionGuidelines().size());
    }

    private boolean processEvent(final IOrganizationEvent organizationEvent) {
        //TODO: Greg: do we use the reorg needed return value?
    ...
    private void processGoalModelModification(
        ...
        for (final InstanceGoal<InstanceParameters> instanceGoal : modifiedGoals) {
            ...
            if (assignment != null) {
                ...
                final ParticipateMessage message = new edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipateMessage(getOwner().getUniqueIdentifier(),
                        assignment.getAgent().getIdentifier(), edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipatePerformative.GOAL_MODIFICATION, content);
    ...
    private void processMessage(final edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipateMessage message) {
    ...
    private void checkAgent(final Agent<?> agent) {
        ...
        for (final Assignment assignment : this.organizationModel.getAssignmentsOfAgent(
            ...
            if (goodness <= RoleGoodnessFunction.MIN_SCORE) {
                ...
                final IBaseMessage message = edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipateMessage.createLocalParticipateMessage(
                        getOwner().getUniqueIdentifier(),
                        assignment.getAgent().getIdentifier(),
                        edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipatePerformative.DEASSIGNMENT,
                        AssignmentContent.createAssignmentContent(assignment));
    ...
    protected void processAssignment(final edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipateMessage message) {
        final Object content = message.getContent();
    ...
    protected void processDeAssignment(final edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipateMessage message) {
        final Object content = message.getContent();
        if (content instanceof AssignmentContent) {
            final AssignmentContent assignmentContent = (AssignmentContent) content;
    ...
    protected void processGoalModification(final edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipateMessage message) {
        final Object content = message.getContent();
        if (content instanceof ModificationContent) {
            final ModificationContent modificationContent = (ModificationContent) content;

            if (debug) LOG.debug("processGoalModification() - {}", String.format("Modification: %s", modificationContent));
    ...
    public synchronized boolean processRegistrationMessages() {
        ...
        if (debug) LOG.debug("Need {} of {} registrations to register sub holons.", childRegistrationsStillNeeded(), tot);
    ...
    public synchronized boolean noChildren() {
        ...
        try {
            ...
            LOG.error("Error checking to see if {} has no sub holons. tictactoeConnections = {}", owner.getIdentifierString(), this.getChildConnections());
            System.exit(-4);
        }
    ...
    private synchronized boolean registerAnyChild() {
        try {
            final edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipateMessage message = remoteRECEIVE(owner.getIdentifierString());
            if (message == null) {
            ...
            getChildConnections().getListConnectionGuidelines().stream().filter(cg ->
                    cg.getOtherAgentAbbrev().equals(sender)).forEach(cg ->
                    cg.setRegistered(true));
            if (debug) LOG.info("Parent {} registered sub holon {}.", receiver, sender);
        } catch (Exception e) {
    ...
    /**
     Checking for remote registration messages. Childs send them with their OrganizationCommunicationCapaability.
     ...
     @throws InterruptedException - Handles all Interrupted Exceptions
     */
    public synchronized edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipateMessage remoteRECEIVE(final String queueLink) throws IOException, ShutdownSignalException, InterruptedException {
        final QueueingConsumer consumer = new QueueingConsumer(Objects.requireNonNull(channel, "Error null channel in receive()."));
        final String fullQueueName = TicTacToeMessagingManager.getFullQueueName(queueLink, TicTacToeMessagingManager.getQueueFocus(messagingFocus));

        TicTacToeMessagingManager.declareAndBindConsumerQueue(messagingFocus, queueLink);
        if (debug) LOG.debug("Checking for remote ParticipateMessage (e.g. registration) on {}", fullQueueName);
        ...
        if (delivery != null) {
            ...
            edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipateMessage message = edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipateMessage.createParticipateMessage();
            if (debug)
            ...
            try {
                message = (edu.ksu.cis.macr.aasis.agent.cc_message.participate.ParticipateMessage) message.deserialize(delivery.getBody());
    ...
    public boolean processingRegistrationIsComplete() {
        return childRegistrationsStillNeeded() == 0;
    }

    /**
     @param tictactoeConnections - the list of all authorized connections to child or subordinate agents.
     */
    public synchronized void setAllChildConnectionGuidelines(final IConnections tictactoeConnections) {
        this.setChildConnections(tictactoeConnections);
    }
}
