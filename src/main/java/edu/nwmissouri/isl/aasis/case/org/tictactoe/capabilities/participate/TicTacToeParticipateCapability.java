package edu.nwmissouri.isl.aasis.case.org.tictactoe.capabilities.participate;
...
public class TicTacToeParticipateCapability extends AbstractOrganizationCapability implements IOrganizationCommunicationCapability, ICapability {
    /**
    ...
    private static final Logger LOG = LoggerFactory.getLogger(TicTacToeParticipateCapability.class);
    private static final String QUEUE_PURPOSE = "_TICTACTOE";
     ...
     */
    public TicTacToeParticipateCapability(final IPersona owner, final IOrganization org) {
        super(TicTacToeParticipateCapability.class, owner, org);
    ...
    public boolean send(final IBaseMessage<?> message) {
        ...
        if (isLocal) {
            ...
            String routingKey = TicTacToeMessagingManager.getFullQueueName(message.getRemoteReceiver(), TicTacToeMessagingManager.getQueueFocus(messagingFocus));
...
//  }
    ...
    public Set<Agent<?>> getLocalRegisteredProsumers(Set<Agent<?>> allAgents) {
        ...
        while (it.hasNext()) {
            Agent<?> agent = it.next();
            LOG.debug("Checking registered persona {} for TicTacToeParticipateCapability", agent.toString());
