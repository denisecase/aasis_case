...
import edu.nwmissouri.isl.aasis.case.primary.capabilities.participate.Participant;
 ...
 */
public class PokerParticipant extends Participant implements IBaseControlComponentSlave {
  private static final Logger LOG = LoggerFactory.getLogger(PokerParticipant.class);
   ...
   */
  public PokerParticipant(final String name, final IPersona persona, final Element knowledge,
      final OrganizationFocus focus) {
    super(name, persona, knowledge, focus);
    LOG.info("Entering constructor PokerParticipant( name={}, persona={}, knowledge={}, focus={}.", name, persona,
        knowledge, focus);
  ...
  protected void doRegistration() {
    if (debug)
      LOG.debug("Beginning doRegistration().");
    ...
    if (debug)
      LOG.debug("In doRegistration() {}",
          String.format("Agent: \"%s\": %s", agent.getIdentifier(), agent.getPossessesSet()));
    ...
    String org = this.getPersonaExecutionComponent()
        .getOrganizationFromPersonaName(getPersonaExecutionComponent().getUniqueIdentifier().toString());
    if (debug)
      LOG.debug("My self org is {}.", org);
    String host = this.getPersonaExecutionComponent()
        .getHostFromIdentifier(getPersonaExecutionComponent().getUniqueIdentifier());
    if (debug)
      LOG.debug("I'm running on host {}.", host);
    if (debug)
      LOG.debug("This is  {} in organization {} running on node {}", this.getOwner().getUniqueIdentifier().toString(),
          org, host);
    this.startTime = System.currentTimeMillis(); // will be used to see if we timeout

    if (debug)
      LOG.debug("Internal persona is attempting to register. org={} node={} ", org, host);
    ...
    if (reasoningMessage != null) {
      if (this.internalOrganizationCommunicationCapability.broadcast(reasoningMessage)) {
        ...
        LOG.debug(
            "REGISTRATION MESSAGE broadcast. {} messages on queue.  Moving to {}.  MESSAGE={}. internal org messages are: {}",
  ...
  protected void doRegistrationConfirmation() {
    if (debug)
      LOG.debug("External SLAVE doRegistrationConfirmation.....");
    if (this.internalOrganizationCommunicationCapability.messages() > 0) {
      ...
      case BROADCASTING_AGENT_REGISTRATION:
         ...
         */
        if (debug)
          LOG.debug("doRegConf() ignoring in order to confirm: {}", message);
        this.state = ExecutionState.TIMING_OUT_DURING_CONFIRMATION;
        break;
      case AGENT_REGISTRATION_CONFIRMATION:
        if (debug)
          LOG.debug("Got AGENT_REGISTRATION_CONFIRMATION. content is {}", message.getContent());
        ...
        if (content instanceof UniqueIdentifier) {
          ...
          if (debug)
            LOG.debug("doRegistrationConfirmation()confirmation from {}", this.master);
