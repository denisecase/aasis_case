 ...
 *
 */
package edu.nwmissouri.isl.aasis.case.primary.persona;
...
import edu.nwmissouri.isl.aasis.case.primary.capabilities.participate.PrimaryCommunicationCapability;
...
public class Persona extends AbstractPersona {
  ...
  protected PrimaryCommunicationCapability localPrimaryCommunicationCapability;
  ...
  public Persona(final String identifierString) {
    ...
    if (debug)
      LOG.debug("\t New playable capability={}.", this.playerCapability);

    this.localPrimaryCommunicationCapability = new PrimaryCommunicationCapability(this, organization);
    if (debug)
      LOG.debug("\t New PrimaryCommunicationCapability={}", this.localPrimaryCommunicationCapability);
    addCapability(localPrimaryCommunicationCapability);
    if (debug)
      LOG.debug("\t Added PrimaryCommunicationCapability={}.", this.localPrimaryCommunicationCapability);
    ...
    this.internalCommunicationCapability.addChannel(localPrimaryCommunicationCapability.getCommunicationChannelID(),
        this.localPrimaryCommunicationCapability);
    if (debug)
      LOG.debug("\t Added localPrimaryCommunicationCapability internally to add the channel.");
   ...
   */
  public Persona(final IOrganization org, final String identifierString, final Element knowledge,
      final OrganizationFocus focus) {
    super(org, identifierString, knowledge, focus);
    LOG.info("\t..................CONSTRUCTING PERSONA(org={}, identifier={}, knowledge={}, focus={}", org,
        identifierString, knowledge, focus);
    this.identifierString = identifierString;
    organization = org;
    this.focus = focus;
    ...
    this.localPrimaryCommunicationCapability = new PrimaryCommunicationCapability(this, org);
    if (debug)
      LOG.debug("\t New PrimaryCommunicationCapability={}", this.localPrimaryCommunicationCapability);
    addCapability(localPrimaryCommunicationCapability);
    if (debug)
    ...
    if (this.internalCommunicationCapability == null) {
      LOG.debug("ERROR: internalCommunicationCapability is null - Can't add localPrimaryCommunicationCapability.");
    } else {
      if (debug)
        LOG.debug("\tlocalPrimaryCommunicationCapability.getCommunicationChannelID()={}.",
            localPrimaryCommunicationCapability.getCommunicationChannelID());
  ...
  public void execute() {
    if (debug)
      LOG.debug("**** Entering EC Execution Algorithm execute(). {} assignments.", assignments());
    while (isAlive()) {
      Player.step();
      /* first: update the ec */
      try {
        while (assignments() > 0) {
          if (debug)
            LOG.debug("Number of Assignments = {}", assignments());
      ...
      try {
        ...
        while (deAssignments() > 0) {
          if (debug)
            LOG.debug("Number of DeAssignments = {}", deAssignments());
      ...
      try {
        ...
        LOG.error("ERROR in EC EXECUTE getNextAssignedTask {}.Exception: {}  {}{}", this.getIdentifierString(),
    ...
    if (goodnessScore > RoleGoodnessFunction.MIN_SCORE) {
      ...
      if (executablePlan == null) {
        ...
        try {
          ...
          LOG.info("Selected plan for {} to achieve {} is {}.", roleIdentifier, goalIdentifier.toString(),
